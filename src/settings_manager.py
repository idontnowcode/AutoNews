...
def check_news_should_run() -> bool:
    ...
    뉴스 파이프라인 실행 여부 판단:
      ...
      - 마지막 뉴스 업로드 이후 interval 미경과 → 건너뜀
    """
    ...
    # 마지막 업로드: news_items 테이블 기준
    db  = get_client()
    res = db.table('news_items').select('created_at') \
            .eq('status', 'done') \
    ...
    if res.data:
        ...
        if elapsed < delta:
            ...
            print(f'⏱️  간격 대기 중: 마지막 뉴스 업로드 후 {int(elapsed.total_seconds()//3600)}h {int((elapsed.total_seconds()%3600)//60)}m 경과 '
